...
interface FinalRoutineProps {
  departmentId?: number;
  teacherId?: number; // Added teacherId prop
}

export default function FinalRoutine({ departmentId, teacherId }: FinalRoutineProps) {
  ...
  const [teacherCourses, setTeacherCourses] = useState<number[]>([]); // Store teacher's course IDs
  ...
  // Fetch teacher's courses if teacherId and myCourses filter is selected
  useEffect(() => {
    if (filterType !== "myCourses" || !teacherId) return;
  ...
  const filteredRoutine = useMemo(() => {
    ...
    if (filterType === "myCourses") return routine.filter(e => teacherCourses.includes(Number(e.courseId)));
        ...
        <>
          ...
          <div className="flex gap-4 mb-4">
            ...
            {(filterType && (filterValue || filterType === "myCourses")) && (
              <button className="btn btn-xs btn-outline ml-2 cursor-pointer custom-bordered-btn" onClick={() => { setFilterType(""); setFilterValue(null); setTeacherCourses([]); }}>Clear Filter</button>
            )}
          </div>
