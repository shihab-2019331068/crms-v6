...
const DepartmentManage: React.FC<DepartmentManageProps> = ({ departmentId }) => {
  const [view, setView] = useState<"rooms" | "labs" | "courses" | "semesters" | "routine" | null>(null);
  ...
  const renderContent = () => {
    switch (view) {
      ...
      case 'labs':
        return <LabList departmentId={departmentId} />;
      case 'courses':
  ...
  return (
    <div>
      <div className="flex gap-4 mb-4">
        ...
        <button className="btn btn-outline btn-sm mt-1 cursor-pointer custom-bordered-btn" onClick={() => setView('labs')}>Labs</button>
